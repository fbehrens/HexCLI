...
//  HexCLI
//
//  Prepares audio files for transcription, including padding short clips for Parakeet.
//

import AVFoundation
import Foundation

/// Result of audio preparation, including a cleanup handler for temporary files.
struct AudioPreparationResult {
    let url: URL
...
/// Prepares audio files for transcription backends.
enum AudioPreparer {
    private enum Error: LocalizedError {
        ...
        var errorDescription: String? {
            ...
            case .bufferAllocationFailed:
                return "Unable to allocate buffer while preparing audio clip."
            }
        }
    }
    ...
    /// Ensures the audio file meets minimum duration requirements for Parakeet.
    /// Short clips are padded with silence.
    static func ensureMinimumDuration(
        url: URL,
        minimumDuration: TimeInterval = defaultMinimumDuration
    ) throws -> AudioPreparationResult {
        let audioFile = try AVAudioFile(forReading: url)
        let format = audioFile.processingFormat
        let duration = Double(audioFile.length) / format.sampleRate

        guard duration < minimumDuration else {
            return AudioPreparationResult(url: url, cleanupURL: nil)
        }
        ...
        let sourceCapacity = max(
            AVAudioFrameCount(min(existingFrames64, AVAudioFramePosition(AVAudioFrameCount.max))),
            1
        )
        ...
        return AudioPreparationResult(url: paddedURL, cleanupURL: paddedURL)
    }

    private static func makePaddedURL(from url: URL) -> URL {
        let base = url.deletingLastPathComponent()
        let stem = url.deletingPathExtension().lastPathComponent
        return base.appendingPathComponent("\(stem)-hex-cli-padded.wav")
    }
}
